...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  const shareListButtonRef = useRef<HTMLButtonElement>(null)
  ...
  const getReleaseYear = () => {
    ...
    if (!date) {
      const unknownTranslations: Record<string, string> = {
        'tr': 'Belirsiz',
        'de': 'Unbekannt',
        'ar': 'غير محدد',
        'es': 'Desconocido',
      ...
      return unknownTranslations[languageCode] ||
          ...
          <div className="p-3 pt-2">
            {hideActions ? (
              ...
              <>
                ...
                <div className={`flex items-center justify-center space-x-2 ${languageCode === 'ar' ? 'space-x-reverse' : ''}`}>
                {/* Watchlist Button */}
                <button 
                  onClick={handleWatchlistAction}
                  disabled={isAddingToWatchlist}
