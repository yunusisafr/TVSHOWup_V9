...
const AIChatDiscovery: React.FC<AIChatDiscoveryProps> = ({ onClose }) => {
  ...
  const getSuggestions = () => {
    switch (languageCode) {
      ...
      case 'ar':
        return [
          'أفلام أكشن على Netflix بتقييم 8+',
  ...
  const initializeConversation = async () => {
    try {
      ...
      const getWelcomeMessage = () => {
        switch (languageCode) {
          case 'tr':
            return 'Merhaba! Size mükemmel film ve dizi önerileri sunmak için buradayım. Ne tür içerik arıyorsunuz?';
          case 'de':
            return 'Hallo! Ich helfe Ihnen dabei, fantastische Filme und Serien zu entdecken. Welche Art von Inhalten suchen Sie?';
          case 'fr':
            return 'Bonjour! Je suis là pour vous aider à découvrir d\'excellents films et séries. Quel type de contenu recherchez-vous?';
          case 'es':
            return '¡Hola! Estoy aquí para ayudarte a descubrir películas y series increíbles. ¿Qué tipo de contenido buscas?';
          case 'ar':
            return 'مرحباً! أنا هنا لمساعدتك في اكتشاف أفلام ومسلسلات رائعة. ما نوع المحتوى الذي تبحث عنه؟';
          default:
            return 'Hello! I\'m here to help you discover amazing movies and TV shows. What kind of content are you looking for?';
        }
      };
    ...
    if (!canProceed) {
      ...
      const errorMessage: ChatMessage = {
        ...
        content: languageCode === 'tr'
          ...
          : languageCode === 'ar'
          ? 'تم الوصول إلى الحد اليومي. يرجى المحاولة لاحقاً.'
  ...
  return (
    <div className="flex flex-col h-full w-full bg-gradient-to-br from-slate-900 via-purple-900/20 to-slate-900 ai-chat-container">
      ...
      <div className="bg-gradient-to-r from-purple-800/40 to-blue-800/40 backdrop-blur-md p-3 sm:p-4 border-b border-purple-500/30 flex items-center justify-between shrink-0 shadow-lg">
        ...
        <div className="flex items-center space-x-1 sm:space-x-2 shrink-0">
          {!isLoadingLimits && usageLimits && (
            <>
              {/* Desktop Version */}
              <div className="hidden sm:flex items-center space-x-1.5 px-3 py-1.5 bg-slate-800/60 rounded-lg border border-purple-500/30 relative">
              ...
              {/* Mobile Version */}
              <div className="sm:hidden flex items-center space-x-1 px-2 py-1 bg-slate-800/60 rounded-lg border border-purple-500/30 relative">
        ...
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 scrollbar-thin scrollbar-thumb-purple-600/50 scrollbar-track-transparent chat-scroll-area">
            {messages.map((message, index) => (
              ...
              {message.role === 'assistant' && message.id && (
                <div className="mt-2 pt-2 border-t border-gray-700 flex items-center space-x-2">
        ...
        {messages.length === 1 && !isProcessing && usageLimits && usageLimits.remaining > 0 && (
          <div className="mt-2 sm:mt-4 px-2">
            <p className="text-xs sm:text-sm text-purple-300 mb-2 text-center font-medium">
              {languageCode === 'tr' ? 'Önerilen aramalar:' : 'Try these suggestions:'}
            </p>
            ...
            <input
              ...
              placeholder={
                usageLimits && usageLimits.remaining <= 0
                  ? (languageCode === 'tr' ? 'Günlük limit doldu...' : 'Daily limit reached...')
                  : (languageCode === 'tr'
                    ? 'Ne tür içerik arıyorsunuz?'
