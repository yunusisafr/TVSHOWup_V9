...
const LanguageRouter: React.FC<LanguageRouterProps> = ({ children }) => {
  ...
  useEffect(() => {
    // Skip language routing for special routes (admin, reset-password, sitemaps)
    const skipLanguageRouting =
      location.pathname.startsWith('/admin') ||
      location.pathname === '/reset-password' ||
      location.pathname.startsWith('/sitemap') ||
      location.pathname === '/robots.txt' ||
      location.pathname === '/ads.txt'

    if (skipLanguageRouting) {
      console.log('⏭️ Skipping language routing for special route:', location.pathname)
      return
    }
