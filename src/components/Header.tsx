...
const Header: React.FC<HeaderProps> = ({ onSearch = () => {} }) => {
  ...
  // Filter countries based on search query (including language code)
  const supportedCountries = getSupportedCountries()

  // Helper function to get language code for a country
  const getCountryLangCode = (countryCode: string): string => {
    const map: Record<string, string> = {
      'US': 'en', 'GB': 'en', 'CA': 'en', 'AU': 'en', 'NZ': 'en', 'IE': 'en', 'ZA': 'en', 'SG': 'en', 'PH': 'en',
      'TR': 'tr',
      'DE': 'de', 'AT': 'de', 'CH': 'de',
      'FR': 'fr', 'BE': 'fr', 'LU': 'fr',
      'ES': 'es', 'MX': 'es', 'AR': 'es', 'CL': 'es', 'CO': 'es', 'PE': 'es', 'VE': 'es', 'UY': 'es', 'EC': 'es', 'BO': 'es', 'PY': 'es',
      'IT': 'it',
      'PT': 'pt', 'BR': 'pt',
      'NL': 'nl',
      'RU': 'ru',
                ...
                >                  
                  ...
                  <div className="max-h-60 overflow-y-auto">
                    {Object.entries(filteredCountries).map(([code, name]) => {
                      // Get language code for this country from UserPreferencesContext
                      // We'll use a simple mapping based on common patterns
                      const getLangCode = (countryCode: string): string => {
                        const map: Record<string, string> = {
                          'US': 'en', 'GB': 'en', 'CA': 'en', 'AU': 'en', 'NZ': 'en', 'IE': 'en', 'ZA': 'en', 'SG': 'en', 'PH': 'en',
                          'TR': 'tr',
                          'DE': 'de', 'AT': 'de', 'CH': 'de',
                      ...
                      const langCode = getLangCode(code);
                      ...
                      return (
                        ...
                        >
                          <span className="mr-2 text-xs font-mono text-gray-500">{code}-{langCode.toUpperCase()}</span>
                          {name}
                        </button>
                      );
                    })}
