...
const HomePage: React.FC = () => {
  ...
  const [contentSections, setContentSections] = useState<{
    ...
    gainContent: ContentItem[],
    exxenContent: ContentItem[],
    tvPlusContent: ContentItem[],
    publicLists: any[]
  }>({
    trending: [],
    netflixContent: [],
  ...
  const loadContent = async () => {
    try {
      ...
      // Add Turkish-only providers if language is Turkish
      if (languageCode === 'tr') {
        loadPromises.push(
          loadContentByProvider(4409, 30), // Gain - 75 total content available
          loadContentByProvider(4405, 30), // Exxen - 68 total content available
          loadContentByProvider(1904, 24)  // TV+ - 43 total content available
        );
      }
      ...
      const [gainData, exxenData, tvPlusData] = languageCode === 'tr' ? turkishProviderData : [[], [], []]
      ...
      console.log(`✅ Loaded content sections:`, {
        ...
        max: maxData.length,
        ...(languageCode === 'tr' && {
          gain: gainData.length,
          exxen: exxenData.length,
          tvPlus: tvPlusData.length
        }),
      ...
      setContentSections({
        ...
        maxContent: maxData,
        gainContent: gainData,
        exxenContent: exxenData,
        tvPlusContent: tvPlusData,
    ...
    <div className="min-h-screen bg-gray-900">
              ...
              <div className="min-w-0 flex-1 text-left">
                ...
                <p className="text-xs sm:text-sm text-gray-300">
                  {languageCode === 'tr' ? 'AI ile mükemmel içerik keşfedin' :
                   ...
                   languageCode === 'ar' ? 'اكتشف محتوى مثالي باستخدام الذكاء الاصطناعي' :
      ...
      <div className="pb-8 pt-6 sm:pt-8">
        <div className="space-y-8">
          ...
          {/* Turkish Platforms - Only shown in Turkish language */}
          {languageCode === 'tr' && (
            <>
              {/* Gain Content Section */}
              {contentSections.gainContent.length > 0 && (
                <div className="py-6 bg-gradient-to-r from-purple-600/30 to-transparent rounded-xl mx-4 sm:mx-6 lg:mx-8 mb-8">
                  <HorizontalContentRow
                    title={t.gainContent}
                    content={contentSections.gainContent}
                    onWatchlistStatusChange={handleWatchlistStatusChange}
                    userWatchlistMap={userWatchlistMap}
                    loading={loading}
                    onAuthRequired={() => openAuthPrompt('watchlist')}
              ...
              {/* Exxen Content Section */}
              {contentSections.exxenContent.length > 0 && (
                <div className="py-6 bg-gradient-to-r from-orange-600/30 to-transparent rounded-xl mx-4 sm:mx-6 lg:mx-8 mb-8">
                  <HorizontalContentRow
                    title={t.exxenContent}
                    content={contentSections.exxenContent}
                    onWatchlistStatusChange={handleWatchlistStatusChange}
                    userWatchlistMap={userWatchlistMap}
                    loading={loading}
                    onAuthRequired={() => openAuthPrompt('watchlist')}
              ...
              {/* TV+ Content Section */}
              {contentSections.tvPlusContent.length > 0 && (
                <div className="py-6 bg-gradient-to-r from-pink-600/30 to-transparent rounded-xl mx-4 sm:mx-6 lg:mx-8 mb-8">
                  <HorizontalContentRow
                    title={t.tvPlusContent}
                    content={contentSections.tvPlusContent}
                    onWatchlistStatusChange={handleWatchlistStatusChange}
                    userWatchlistMap={userWatchlistMap}
                    loading={loading}
                    onAuthRequired={() => openAuthPrompt('watchlist')}
