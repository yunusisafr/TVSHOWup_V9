  ...
  const loadContent = async () => {
    try {
      ...
      if (!dbContent) {
        ...
        try {
          // Fetch directly from TMDB API - FIRST in ENGLISH for proper slug generation and credits
          const tmdbType = contentType === 'tv_show' ? 'tv' : 'movie'
          const tmdbUrlEn = `https://api.themoviedb.org/3/${tmdbType}/${contentId}?api_key=${import.meta.env.VITE_TMDB_API_KEY}&language=en&append_to_response=credits,keywords,watch/providers`

          const tmdbResponseEn = await fetch(tmdbUrlEn)

          if (!tmdbResponseEn.ok) {
            throw new Error(`TMDB API error: ${tmdbResponseEn.status}`)
          }

          const tmdbDataEn = await tmdbResponseEn.json()

          // If user language is not English, fetch again in their language for translated content
          let tmdbData = tmdbDataEn
          if (languageCode !== 'en') {
            const tmdbUrlLang = `https://api.themoviedb.org/3/${tmdbType}/${contentId}?api_key=${import.meta.env.VITE_TMDB_API_KEY}&language=${languageCode}&append_to_response=credits`
            const tmdbResponseLang = await fetch(tmdbUrlLang)
            if (tmdbResponseLang.ok) {
              const tmdbDataLang = await tmdbResponseLang.json()
              // Merge: Keep English credits (cast/crew names don't change), use user language for text content
              tmdbData = {
                ...tmdbDataEn, // Keep ALL English data including credits
                overview: tmdbDataLang.overview || tmdbDataEn.overview,
                tagline: tmdbDataLang.tagline || tmdbDataEn.tagline,
                genres: tmdbDataLang.genres || tmdbDataEn.genres,
                credits: tmdbDataEn.credits // Explicitly use English credits to ensure they're available
              }
            }
          }
      ...
      if (!providerData || providerData.length === 0 || updateCheck.shouldUpdateProviders) {
        ...
        try {
          ...
          if (tmdbProvidersResponse.ok) {
            ...
            const { data: networkProviderData } = await databaseService.supabase
              .from('content_providers')
              .select(`
                *,
                providers (
              ...
              .eq('content_type', content.content_type)
              .eq('country_code', countryCode)
              .eq('providers.provider_type', 'network')
              .not('providers.name', 'is', null)

            const networkProviders = (networkProviderData || [])
              .filter(item => item.providers && item.providers.name)
              .map(item => ({
            ...
            const allCombinedProviders = [...tmdbProviders, ...networkProviders]
              .sort((a, b) => {
            ...
            setProviders(allCombinedProviders)
            setAllProviders(allCombinedProviders)
            console.log(`✅ Loaded ${tmdbProviders.length} streaming + ${networkProviders.length} network providers for ${countryCode}`)
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="mt-4 xs:mt-6 sm:mt-8 pt-3 xs:pt-4 sm:pt-6 border-t border-white/20">
                  <div className="flex items-center mb-4">
                    <Play className="w-4 h-4 xs:w-5 xs:h-5 mr-2 text-primary-400" />
                  ...
                  {providers.length > 0 ? (
                    ...
                    <div className="grid grid-cols-2 xs:grid-cols-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-3">
                      {providers.slice(0, showAllProviders ? providers.length : 6).map((provider) => (
                        <div 
                          key={provider.id} 
                          className="flex items-center space-x-2 sm:space-x-3 bg-black/30 rounded-lg p-2 xs:p-2.5 sm:p-3 hover:bg-black/50 transition-colors"
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="text-gray-300 text-xs capitalize">
                              {provider.provider_type === 'network' ? 
                                (languageCode === 'tr' ? 'TV Ağı' : 'TV Network') :
                                provider.monetization_type === 'flatrate' ? 
                                (languageCode === 'tr' ? 'Streaming' : 'Streaming') : 
                                provider.monetization_type === 'buy' ? 
                                (languageCode === 'tr' ? 'Satın Al' : 'Buy') :
                                provider.monetization_type === 'rent' ? 
                                (languageCode === 'tr' ? 'Kirala' : 'Rent') :
                                provider.monetization_type === 'ads' ? 
                                (languageCode === 'tr' ? 'Reklamlı' : 'With Ads') :
                    ...
                    <div className="bg-gray-800/50 rounded-lg p-4 text-center">
                      <p className="text-gray-300 text-sm">
                        {t.contentNotAvailable.replace('{country}', countryCode)}
                      </p>
                    </div>
                  )}
                </div>
