...
const PersonDetailPage: React.FC = () => {
  ...
  const loadPersonDetails = async () => {
    try {
      ...
      const personData = await tmdbService.getPersonDetails(parseInt(actualId!), languageCode)
  ...
  const updateMetaData = () => {
    ...
    if (metaDescription) {
      ...
      if (person.biography) {
        ...
        description = languageCode === 'tr'
          ? `${personName}${birthInfo} - ${knownFor}, filmografi, biyografi ve detaylı bilgiler - TVSHOWup`
          : `${personName}${birthInfo} - ${knownFor}, filmography, biography and detailed information - TVSHOWup`
      }
  ...
  const getGender = () => {
    ...
    switch (person.gender) {
      case 1: return languageCode === 'tr' ? 'Kadın' : 'Female'
      case 2: return languageCode === 'tr' ? 'Erkek' : 'Male'
      case 3: return languageCode === 'tr' ? 'Diğer' : 'Non-binary'
      default: return languageCode === 'tr' ? 'Belirtilmemiş' : 'Not specified'
    }
  }
  ...
  const getKnownFor = () => {
    if (!person?.known_for_department) return 'Acting'
    switch (person.known_for_department) {
      case 'Acting': return languageCode === 'tr' ? 'Oyunculuk' : 'Acting'
      case 'Directing': return languageCode === 'tr' ? 'Yönetmenlik' : 'Directing'
      case 'Writing': return languageCode === 'tr' ? 'Senaristlik' : 'Writing'
      case 'Production': return languageCode === 'tr' ? 'Yapımcılık' : 'Production'
      case 'Sound': return languageCode === 'tr' ? 'Ses' : 'Sound'
      case 'Camera': return languageCode === 'tr' ? 'Kamera' : 'Camera'
      case 'Editing': return languageCode === 'tr' ? 'Kurgu' : 'Editing'
      case 'Art': return languageCode === 'tr' ? 'Sanat Yönetimi' : 'Art Direction'
      case 'Costume & Make-Up': return languageCode === 'tr' ? 'Kostüm & Makyaj' : 'Costume & Make-Up'
      case 'Visual Effects': return languageCode === 'tr' ? 'Görsel Efekt' : 'Visual Effects'
      ...
      <div className="min-h-screen bg-gray-900 flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-white mb-4">
            {languageCode === 'tr' ? 'Kişi Bulunamadı' : 'Person Not Found'}
          </h1>
          <p className="text-gray-400 mb-8">
            {error || (languageCode === 'tr' ? 'İstenen kişi bulunamadı.' : 'The requested person could not be found.')}
          </p>
          <button
            onClick={() => navigate(-1)}
            className="bg-primary-500 hover:bg-primary-600 text-white px-6 py-3 rounded-lg transition-colors"
          >
            {languageCode === 'tr' ? 'Geri Dön' : 'Go Back'}
          </button>
        </div>
      </div>
    )
          ...
          <div className="space-y-6">
              ...
              {person.profile_path ? (
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <div className="text-gray-500 text-center">
                    ...
                    <p>{languageCode === 'tr' ? 'Fotoğraf Yok' : 'No Photo'}</p>
                  </div>
                </div>
              )}
            </div>
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              <h2 className="text-xl font-bold text-white mb-4">
                {languageCode === 'tr' ? 'Kişisel Bilgiler' : 'Personal Info'}
              </h2>
              
              <div className="space-y-3">
                <div>
                  <span className="text-gray-400 text-sm">
                    {languageCode === 'tr' ? 'Bilinen Alan:' : 'Known For:'}
                  </span>
                ...
                <div>
                  <span className="text-gray-400 text-sm">
                    {languageCode === 'tr' ? 'Cinsiyet:' : 'Gender:'}
                  </span>
                ...
                {person.birthday && (
                  <div>
                    <span className="text-gray-400 text-sm">
                      {languageCode === 'tr' ? 'Doğum Tarihi:' : 'Birthday:'}
                    </span>
                    <div className="flex items-center text-white">
                      ...
                      {getAge() && (
                        <span className="text-gray-400 ml-2">
                          ({getAge()} {languageCode === 'tr' ? 'yaşında' : 'years old'})
                        </span>
                      )}
                    </div>
                  </div>
                ...
                {person.deathday && (
                  <div>
                    <span className="text-gray-400 text-sm">
                      {languageCode === 'tr' ? 'Ölüm Tarihi:' : 'Deathday:'}
                    </span>
                    <div className="flex items-center text-white">
                      <Calendar className="w-4 h-4 mr-2" />
                ...
                {person.place_of_birth && (
                  <div>
                    <span className="text-gray-400 text-sm">
                      {languageCode === 'tr' ? 'Doğum Yeri:' : 'Place of Birth:'}
                    </span>
                    <div className="flex items-center text-white">
                ...
                {person.also_known_as && person.also_known_as.length > 0 && (
                  <div>
                    <span className="text-gray-400 text-sm">
                      {languageCode === 'tr' ? 'Diğer Adları:' : 'Also Known As:'}
                    </span>
          ...
          <div className="lg:col-span-2 space-y-8">
              ...
              {person.biography && (
                <div>
                  <h2 className="text-2xl font-bold text-white mb-4">
                    {languageCode === 'tr' ? 'Biyografi' : 'Biography'}
                  </h2>
                  <div className="text-gray-300 leading-relaxed whitespace-pre-line">
                    {person.biography || (languageCode === 'tr' ? 'Biyografi bilgisi mevcut değil.' : 'No biography available.')}
                  </div>
                </div>
              )}
            </div>
            ...
            <div>
              <h2 className="text-2xl font-bold text-white mb-6">
                {languageCode === 'tr' ? 'Bilinen Yapımlar' : 'Known For'}
              </h2>
              ...
              <div>
                  ...
                  if (allCredits.length > INITIAL_DISPLAY_COUNT && !isShowingAll) {
                    return (
                      <div className="text-center mt-6">
                        <button
                          ...
                          className="bg-primary-500 hover:bg-primary-600 text-white px-6 py-3 rounded-lg transition-colors"
                        >
                          {languageCode === 'tr' ? `Daha Fazla Göster (${allCredits.length - INITIAL_DISPLAY_COUNT} daha)` :
                           languageCode === 'de' ? `Mehr anzeigen (${allCredits.length - INITIAL_DISPLAY_COUNT} weitere)` :
                           languageCode === 'fr' ? `Afficher plus (${allCredits.length - INITIAL_DISPLAY_COUNT} de plus)` :
                           languageCode === 'es' ? `Mostrar más (${allCredits.length - INITIAL_DISPLAY_COUNT} más)` :
                           languageCode === 'it' ? `Mostra di più (${allCredits.length - INITIAL_DISPLAY_COUNT} in più)` :
                           `Show More (${allCredits.length - INITIAL_DISPLAY_COUNT} more)`}
                        </button>
                      </div>
                    )
                    ...
                    return (
                      <div className="text-center mt-6">
                        ...
                        >
                          {languageCode === 'tr' ? 'Daha Az Göster' :
                           languageCode === 'de' ? 'Weniger anzeigen' :
                           languageCode === 'fr' ? 'Afficher moins' :
                           languageCode === 'es' ? 'Mostrar menos' :
                           languageCode === 'it' ? 'Mostra meno' :
                           'Show Less'}
                        </button>
                      </div>
                    )
                ...
                {!loadingProviders && contentWithProviders.length === 0 && getTopCredits(activeTab).length === 0 && (
                  <div className="text-center py-8 text-gray-400">
                    <p>
                      {languageCode === 'tr' 
                        ? 'Bu kategoride yapım bulunamadı.' 
