...
function getSessionId(): string {
  // First check localStorage (new storage location)
  let sessionId = localStorage.getItem('aiChatSessionId');

  // If not in localStorage, check sessionStorage for migration
  if (!sessionId) {
    ...
    if (sessionId) {
      ...
      sessionStorage.removeItem('aiChatSessionId');
      console.log('🔄 Migrated session ID from sessionStorage to localStorage');
    }
  }
  ...
  if (!sessionId) {
    ...
    localStorage.setItem('aiChatSessionId', sessionId);
    console.log('🆕 Created new guest session ID:', sessionId);
  }
...
export const rateLimitService = {
  ...
  async trackPageView(userId: string | null): Promise<void> {
    ...
    const lastView = localStorage.getItem('lastPageViewReward');
    const now = Date.now();
    ...
    if (success) {
      localStorage.setItem('lastPageViewReward', now.toString());
    }
  },
  ...
  async trackAdClick(userId: string | null): Promise<void> {
    ...
    const lastClick = localStorage.getItem('lastAdClickReward');
    const now = Date.now();
    ...
    if (success) {
      localStorage.setItem('lastAdClickReward', now.toString());
    }
  },
