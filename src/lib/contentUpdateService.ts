  ...
  async updateProviderData(
    ...
    try {
      ...
      const { error: deleteError } = await databaseService.supabase
        ...
        .in('provider_id', allProviders.map(p => p.provider_id))

      if (deleteError) {
        console.error(`❌ Error deleting old streaming providers:`, deleteError)
      }
      ...
      console.log(`✅ Saved ${allProviders.length} streaming providers for ${contentType} ${contentId} in ${countryCode} (network providers preserved)`)
    } catch (error) {
