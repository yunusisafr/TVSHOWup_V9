          ...
          if (contentType === 'movie') {
            ...
            if (details.networks && details.networks.length > 0) {
              ...
              for (const network of details.networks) {
                const countryCode = network.origin_country || details.origin_country?.[0] || 'TR'
                const supportedCountries = [countryCode]

                if (details.origin_country && Array.isArray(details.origin_country)) {
                  details.origin_country.forEach(c => {
                    if (c && !supportedCountries.includes(c)) {
                      supportedCountries.push(c)
                    }
                  })
                }
                ...
                await supabaseClient
                  ...
                  .upsert({
                    ...
                    display_priority: network.display_priority || 999,
                ...
                for (const country of supportedCountries) {
                  await supabaseClient
                    ...
                    .upsert({
                      ...
                      data_source: 'tmdb_network'
                    }, {
                      onConflict: 'content_id,content_type,provider_id,country_code,monetization_type'
                    })
                }

                console.log(`✅ Added network ${network.name} (ID: ${network.id}) for countries: ${supportedCountries.join(', ')}`)
              }
            }
